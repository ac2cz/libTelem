...
public class DisplayModule extends JPanel implements ActionListener, MouseListener {
	...
	public DisplayModule(ByteArrayLayout layout, String title, int size, int modType, int displayModuleFontSize, Spacecraft sat, SatTelemStore db) {
		...
		scale = (double)(displayModuleFontSize)/(double)(DEFAULT_FONT_SIZE);	
		//Log.println("SCALE: " + scale + " font:" +displayModuleFontSize + " def: " + DEFAULT_FONT_SIZE);
		setDefaultSizes();
		moduleType = modType;
				
		if (moduleType == DISPLAY_HERCI || moduleType == DISPLAY_HERCI_HK || moduleType == DISPLAY_HERCI_MICRO_PKT) {
			border.setTitleFont(new Font("SansSerif", Font.BOLD, (int)(displayModuleFontSize * 12/11)));
			border.setTitleColor(herciFontColor);
		} else if (moduleType == DISPLAY_MEASURES) {
			border.setTitleFont(new Font("SansSerif", Font.BOLD, (int)(displayModuleFontSize * 12/10)));
	...
	public void addName(int i, String name, String fieldName, String desc, int display) {
		...
		this.label[i].setFont(new Font("SansSerif", Font.PLAIN, displayModuleFontSize));
	...
	public void updateValues(DataRecord rt) throws NumberFormatException, IOException, DataLoadException {
		...
		for (int i=0; i < size; i++) {
			if(fieldName[i] != null) {
				rtValue[i].setFont(new Font("SansSerif", Font.PLAIN, displayModuleFontSize));
				if (rt.hasFieldName(fieldName[i])) { 
					if (displayRawValues)
						rtValue[i].setText(Integer.toString(rt.getRawValue(fieldName[i])));
					else
						rtValue[i].setText(rt.getStringValue(fieldName[i]));
	...
	private void initGui() {
		...
		rtTitle.setFont(new Font("SansSerif", Font.BOLD, displayModuleFontSize));	
		...
		if (moduleType == DISPLAY_MEASURES) {
			...
			for (int i=1; i < size; i++) {
				...
				minValue[i].setFont(new Font("SansSerif", Font.PLAIN, displayModuleFontSize));
		...
		if (moduleType < DISPLAY_VULCAN) {
			...
			minTitle.setFont(new Font("SansSerif", Font.BOLD, displayModuleFontSize));	
			row[0].add(minTitle);
			...
			maxTitle.setFont(new Font("SansSerif", Font.BOLD, displayModuleFontSize));	
			...
			for (int i=1; i < size; i++) {
				...
				maxValue[i].setFont(new Font("SansSerif", Font.PLAIN, displayModuleFontSize));
